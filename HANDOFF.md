...
- ~~BUG-WS-2 fix~~ — Push-to-AI JSON-parse failure ("Unterminated string in JSON at position 3713") · max_tokens passthrough on Anthropic/OpenAI/Gemini · repairTruncatedJson 5-step recovery (escape stray newlines · close unterminated strings + braces · strip trailing commas) · retry-once with strict-JSON discipline reminder · 6 regression tests added · 1329/1329 GREEN · shipped at `8b845a4`
- ~~BUG-WS-3 fix~~ — [Import to canvas] showed misleading "Push notes to AI first" toast AFTER successful push · 3-state discrimination (never-pushed / pushed-zero-emitted / pushed-all-dropped) + lastPushedAt/lastDroppedCount tracking + showOverlayError in-overlay banner (not 3.5s toast) + actionable remediation guidance · V-FLOW-WS-IMPORT-ZERO-MAPPINGS-1 NEW · shipped at `12e178b`
- ~~BUG-WS-4 fix~~ — repairTruncatedJson didn't handle dangling-key truncation `{"key":"val", "key2"}` · Step 6 added (2 regex patterns + 3 iteration passes + re-strip trailing commas) · V-FLOW-WS-PARSE-REPAIR-1 extended with 3 BUG-WS-4 guards · shipped at `12e178b`
- ~~BUG-WS-5 fix~~ — parseLlmResponse rejected valid JSON followed by trailing prose / closing fence / rationale block · NEW extractFirstBalancedJson helper (inside-string-aware brace counter) called at Step 0.5 · V-FLOW-WS-PARSE-REPAIR-1 extended with 4 BUG-WS-5 guards · shipped at `12e178b`
...
| 42 | `c1376d5` | docs · session log addendum + HANDOFF.md refreshed for BUG-WS-1 + Step 6 eval forensic | **Phase 5 (doc)** |
| 43 | `8b845a4` | fix · BUG-WS-2 · Push-to-AI JSON-parse failure resolved · max_tokens override (Anthropic/OpenAI/Gemini) + repairTruncatedJson + retry-once + 6 regression tests + BUG_LOG entry · 1329/1329 GREEN | **Phase 6 (BUG-FIX)** |
| 44 | `daaa8bd` | docs · Phase 6 addendum + HANDOFF.md refreshed for BUG-WS-2 | **Phase 6 (doc)** |
| 45 | `12e178b` | fix · BUG-WS-3/4/5 · [Import to canvas] 3-state UX + dangling-key repair + trailing-prose extraction · V-FLOW-WS-IMPORT-ZERO-MAPPINGS-1 NEW + V-FLOW-WS-PARSE-REPAIR-1 extended to 10 guards + 3 BUG_LOG entries · 1330/1330 GREEN | **Phase 7 (BUG-FIX)** |

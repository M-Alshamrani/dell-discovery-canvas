...
- ✅ **A4. v2.4.12 Services scope + Pre-flight regression fixes — IMPLEMENTED 2026-04-26.** Full spec in `docs/CHANGELOG_PLAN.md § v2.4.12`. Section S (services): `gap.services[]` field + 10-entry `SERVICE_TYPES` catalog + opt-in suggested chips per gapType + Tab 4 gap-detail multi-chip + Tab 5.5 project-card chip row + NEW Reporting "Services scope" sub-tab. Section PR (regression fixes): PR1 ContextView no-op Save no longer flips `isDemo` (browser smoke R1 ✅), PR2 skillStore CRUD emits `skills-changed` + UseAiButton subscribes (browser smoke R3/R4 ✅). Section U (subtraction): U1 removed the v2.4.11 D2 `+ Add operational / services gap` CTA. Tests: Suite 43 SVC1-10 + PR1.a/b + PR2.a/b/c + U1 + DS23 (17 new tests, all green; total 526/526). Browser smoke R1-R10 all PASS.

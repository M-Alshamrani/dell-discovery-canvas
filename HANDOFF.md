...
**Last session end**: 2026-04-28 (polish iteration 2). **v2.4.15 IMPLEMENTED locally with Tier 1 + Tier 2 polish absorbed** — dynamic environments + soft-delete + vendor mix segmented bar + modern collapsible FilterBar with all 4 dims wired + session capsule polish + footer alignment + matrix tweaks PLUS: GPLC `.tag[data-t]` primitive across the app, button-feedback contract (idle/loading/success/error), AI provider Save fix, blue-dot glitch replaced with Lucide lock + tooltip, hide-flow drops env column entirely (was greyed) and uses Overlay.js for the confirmation modal, FilterBar accordion with persistent per-dim collapse, vendor mix dimension picker (Vendor/Layer/Env) + slow shimmer + click-to-cross-filter. 584 GREEN / 0 RED. **NOT pushed** per `feedback_no_push_without_approval.md` — awaiting user live-smoke + "tag it" approval. v2.4.14 was the previous tag (origin/main HEAD `dd6974b`).

...
**Amendment 2026-05-14 evening (Step 3.6 post-368d565 baseline)**: Step 3.5 hit aggregate target (avg 7.4 · 80% pass · commit `368d565`) but surfaced 2 residual concerns: (R1) ACT-INST-CUR-1 stayed 0/10 with a NEW failure mode — chat grounds + describes payload but asks "Would you like me to propose...?" instead of just emitting on unambiguous v1-kind input; (R2) payload optional-field completeness gap on add-driver (priority missing · 9/10) + add-instance-desired (vendor/vendorGroup/disposition/originId missing · 8/10). Step 3.6 closes both: Rule 4 extended with sentence-append forbidding ask-permission ("Asking the engineer permission to invoke proposeAction ... is the same contract violation as describing the action in prose; ... Just invoke the tool."); proposeAction tool description in `services/chatTools.js` extended with a "**Recommended fields per kind**" block paralleling the Step 3.5 "Required fields per kind" block (priority on add-driver; vendor + vendorGroup + criticality on add-instance-current; vendor + vendorGroup + disposition + originId on add-instance-desired); Behavior Examples section gains Example 12 (canonical add-instance-current tool-use pattern · Veritas NetBackup at DR Site · counter-pattern to ACT-INST-CUR-1's ask-permission failure · intentionally distinct from the ACT-INST-CUR-1 Commvault/Branch-Clinic fixture to prevent memorized-text pattern matching) + header count bumped 11 → 12 worked examples. The amendment changes no plumbing (tool name unchanged · envelope unchanged · `ActionProposalSchema` unchanged · 4-kind enum unchanged · CH38 sub-rules (b)..(g) unchanged); only the steering text tightens. RED-first scaffolds: V-AI-EVAL-16 (Rule 4 "Asking the engineer permission" + "Just invoke the tool" source-grep) + V-AI-EVAL-17 (Example 12 + add-instance-current tool-use pattern + "12 worked examples" header source-grep) + V-CHAT-D-4 ("Recommended fields per kind" + priority + vendorGroup + disposition source-grep). Expected lift target: per-category 9/0/8/10/10 → ~9/8/9/10/10 (ACT-INST-CUR-1 0 → ≥7 · payload-completeness lifts add-driver 9→10 + add-instance-desired 8→9) · passRate 80% → ≥90% · avg score 7.4 → ≥8.5. Step 3.6 commit sequence is (a) preamble + RED (this commit) → (b) impl flips RED → GREEN → (c) user-run re-baseline measures the lift against `368d565`.

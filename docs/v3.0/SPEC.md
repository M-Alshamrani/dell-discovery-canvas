...
**Amendment 2026-05-14 evening (Step 3.5 post-d73ce60 baseline)**: Rule 4 verb-strength tightened from advisory "invoke the `proposeAction` tool" to imperative "MUST invoke the `proposeAction` tool" with explicit "contract violation" framing for prose-only proposal description. Tool-description in `services/chatTools.js` hardened with imperative "MUST be invoked" + per-kind required-fields callout (specifically: `closeReason` field marked REQUIRED on `close-gap` kind). Behavior Examples section gains Example 11 (canonical add-driver tool-use pattern · header count bumped 10 → 11). The amendment is motivated by the rc.10 first action-correctness baseline (`tests/aiEvals/baseline-action.json` · commit `d73ce60` · 3.8/10 avg · 40% pass rate): 3 of 4 emit-cases scored 0/10 by no-emission (chat described actions in prose without invoking the tool), and ACT-INST-DES-1 specifically wrote "Proposal submitted ✓" in prose with `proposedActions: []` (tool-invocation hallucination). The amendment changes no plumbing (tool name unchanged · envelope unchanged · `ActionProposalSchema` unchanged · 4-kind enum unchanged · CH38 7 sub-rules unchanged); only the steering text is tightened. RED-first scaffolds: V-AI-EVAL-13 (Rule 4 `MUST invoke` + `contract violation` source-grep) + V-AI-EVAL-14 (Example 11 + `11 worked examples` header + add-driver tool-use pattern source-grep) + V-CHAT-D-3 (`MUST be invoked` + `closeReason ... REQUIRED` source-grep) + V-AI-EVAL-15 (ACT-INST-CUR-1 prompt rewritten to use Commvault HyperScale X at Branch Clinic · demo-pre-population fidelity bug fix). Expected lift target: per-category averages 0/0/0/9/10 → ~6/6/6/9/10 · passRate 40% → ≥80% · avg 3.8 → ≥7.0. The Step 3.5 commit sequence is (a) preamble + RED (this commit) → (b) impl flips RED → GREEN → (c) user-run re-baseline measures the lift against `d73ce60`.

**Amendment 2026-05-14 evening (Step 3.8 post-3f8ff07 25-case baseline · Example 13 add-instance-desired canonical · completing 3-of-4 emit-kind coverage) — REVERTED post-20:19 25-case re-baseline regression**: Step 3.8 added Example 13 (canonical add-instance-desired pattern · PowerScale H700 replacing HPE 3PAR at DR Site) attempting to lift add-instance-desired (4.0 → ≥7.0 target). The 20:19 re-baseline (`tests/aiEvals/baseline-action-2026-05-14T20-19-28-974Z.json` DIAGNOSTIC only · `ae33705` commit) showed avg 7.4 → 5.92 · pass 76% → 56% · add-instance-current REGRESSED 6.0 → 2.2 (the Step 3.7 win VANISHED) · add-instance-desired stayed at 2.6 (no lift). User strategic pivot direction: the prompt-text iteration cycle (Step 3.5 → 3.6-attempt-revert → 3.7 → 3.8) produced VOLATILE results (avg swung 6.68 → 7.4 → 5.92 on same 25-case set) · the iteration is chasing noise more than signal at this Layer 1 density. Step 3.8 REVERTED: Example 13 + "13 worked examples" header rolled back to Step 3.7 state (Example 11 rewrite + Example 12 + "12 worked examples"). V-AI-EVAL-20 RETIRED (contract surface gone). V-AI-EVAL-18 Guard 3 future-proofing (count >= 12) STAYS · V-AI-EVAL-14 Guard 3 (count >= 11) STAYS · V-AI-EVAL-19 negative guard STAYS. Canonical `tests/aiEvals/baseline-action.json` UNCHANGED · still points at `3f8ff07` Step 3.7 capture (7.4/10 · 76% pass · the last validated improvement). Strategic next moves: (a) scaffold chat-says-vs-chat-does guard at chatService layer (defensive UX · detects "I've proposed" prose with empty proposedActions[] · surfaces warning for preview-modal at Step 5) · (b) eval-methodology calibration (Experiment A: same-prompt-twice for sampling-noise test · Experiment B: same-prompt-but-Gemini-as-judge for same-model-contamination test) · (c) proceed to Step 4 user-facing impl preamble at 3f8ff07 baseline.

**Amendment 2026-05-14 evening (Step 3.8 post-3f8ff07 25-case baseline · Example 13 add-instance-desired canonical · completing 3-of-4 emit-kind coverage) — REVERTED · ORIGINAL TEXT BELOW preserved for historical record**: The Step 3.7 re-baseline (`3f8ff07` · 7.4/10 · 76% pass · 25-case canonical) confirmed the "every emit-kind needs an example" hypothesis: add-instance-current category lifted 2.0→6.0 with Example 12, but add-instance-desired regressed 6.6→4.0 (ACT-INST-DES-1 went 7→0 hallucinating "Done, I've proposed..." without firing; ACT-INST-DES-3 went 8→2 with same hallucination shape · both have the d73ce60 ACT-INST-DES-1 failure-class signature). The forensic mapping across 4 baselines is now well-grounded: add-driver works because Example 11 teaches it; add-instance-current works because Example 12 teaches it; add-instance-desired hallucinates because NO example teaches it; close-gap works without an example (structurally simpler · 3 required fields). Step 3.8 closes the gap: Example 13 NEW (canonical add-instance-desired tool-use pattern · replace-with-originId scenario · PowerScale H700 replacing HPE 3PAR at DR Site · proven-safe short-form notation matching Examples 11+12 from Step 3.7 · intentionally distinct from ACT-INST-DES-1's PowerProtect-DM-replacing-Veeam fixture to prevent memorized-text matching). Header count bumped 12 → 13 worked examples. V-AI-EVAL-19 negative guard (forbids `*[invokes proposeAction(`) stays in place · forward-protection unchanged. V-AI-EVAL-14 Guard 3 future-proofing floor (count >= 11) continues to pass (count=13 ≥ 11). RED-first scaffold: V-AI-EVAL-20 (positive guard · Example 13 marker + add-instance-desired pattern + "13 worked examples" header · 3 guards). NO changes to: tool name, envelope, schema, 4-kind enum, CH38 sub-rules (b)..(g), Rule 4 wording, chatTools.js, chatService.js, Examples 1-12. Expected lift target post-impl against the 25-case canonical: add-instance-desired 4.0 → ≥7.0 (recover from Step 3.7 regression) · passRate 76% → ≥84% · avg 7.4 → ≥7.8. Step 3.8 commit sequence: (a) preamble + RED (this commit) → (b) impl flips RED → GREEN → (c) user-run re-baseline measures the lift against `3f8ff07`.

**Amendment 2026-05-14 evening (Step 3.7 post-5466ea3 25-case baseline · Example 12 RETRY with safe notation)**: The 25-case D4 baseline (`5466ea3` · 6.68/10 avg · 68% pass) revealed that add-instance-current is a CATEGORY-WIDE failure (1/5 pass · 4 emit-cases scored 0/10 by no-emission · including ACT-INST-CUR-3 gold-standard VMware vSphere 7 input). The 5-case foundation's optimistic 7.4/10/80% pass MASKED this because only 1 of 5 cases was add-instance-current. Root cause: Layer 1 has Example 11 (add-driver canonical tool-use) but NO equivalent worked example for add-instance-current; the model has no pattern to imitate for this kind. Step 3.6 attempted Example 12 with `*[invokes proposeAction(kind='X', payload={...}, ...)]*` inline-args notation that the LLM imitated literally as prose; reverted at `58b27c3`. Step 3.7 RETRIES Example 12 using the **proven-safe short-form notation** from Example 8 (`*[calls X for Y: bullets]*`) which has been in Layer 1 since Sub-arc B without imitation issues. The discriminator: short-form `*[calls X]*` (no inline args · no function-call mirror syntax · just verb + short description) vs Step 3.6's full-args inline form. Fixture: Veritas NetBackup at DR Site, tier-2, Medium criticality (intentionally distinct from ACT-INST-CUR-1's Commvault/Branch-Clinic fixture · same anti-memorization design as Step 3.6). Header count bumped 11 → 12 worked examples. RED-first scaffolds: V-AI-EVAL-18 (positive guard · Example 12 marker + add-instance-current pattern within 1500 chars + "12 worked examples" header · 3 guards) + V-AI-EVAL-19 (negative guard · source MUST NOT contain `*[invokes proposeAction(` · forward-protection against re-introducing the Step-3.6 imitation hazard · 1 guard). NO changes to: tool name, envelope, schema, 4-kind enum, CH38 sub-rules (b)..(g), Rule 4 wording, chatTools.js. Only adds Example 12 + header bump. Expected lift target post-impl against the 25-case set: add-instance-current category 2.0 → ≥6.0 (mirror add-driver's lift from Example 11) · passRate 68% → ≥80% · avg 6.68 → ≥7.5. Step 3.7 commit sequence: (a) preamble + RED (this commit) → (b) impl flips RED → GREEN → (c) user-run re-baseline measures the lift against `5466ea3`.

**Amendment 2026-05-14 evening (Step 3.6 post-368d565 baseline) — REVERTED post-cdd367a regression analysis**: Step 3.5 hit aggregate target (avg 7.4 · 80% pass · commit `368d565`) but surfaced 2 residual concerns: (R1) ACT-INST-CUR-1 stayed 0/10 with a NEW failure mode — chat grounds + describes payload but asks "Would you like me to propose...?" instead of just emitting on unambiguous v1-kind input; (R2) payload optional-field completeness gap on add-driver (priority missing · 9/10) + add-instance-desired (vendor/vendorGroup/disposition/originId missing · 8/10). Step 3.6 ATTEMPTED to close both: Rule 4 extended with sentence-append forbidding ask-permission ("Asking the engineer permission to invoke proposeAction ... is the same contract violation as describing the action in prose; ... Just invoke the tool."); proposeAction tool description in `services/chatTools.js` extended with a "**Recommended fields per kind**" block paralleling the Step 3.5 "Required fields per kind" block (priority on add-driver; vendor + vendorGroup + criticality on add-instance-current; vendor + vendorGroup + disposition + originId on add-instance-desired); Behavior Examples section gained Example 12 (canonical add-instance-current tool-use pattern · Veritas NetBackup at DR Site) + header count bumped 11 → 12 worked examples. **REVERTED 2026-05-14 evening per Step 3.6 re-baseline (`tests/aiEvals/baseline-action-2026-05-14T17-44-07-909Z.json` · DIAGNOSTIC only · canonical `baseline-action.json` NOT overwritten): the re-baseline showed a REGRESSION (avg 7.4 → 6.0 · passRate 80% → 60%) caused by the chat IMITATING Example 12's inline `*[invokes proposeAction(...)]*` notation as prose text instead of firing the tool (ACT-INST-CUR-1 chat answer literally wrote `*[invokes proposeAction(kind='add-instance-current'...)]*` in its response body · `proposedActions: []`) AND hallucinating tool invocation on ACT-INST-DES-1 ("Perfect — I've proposed PowerProtect Data Manager..." with empty proposedActions · 8/10 → 0/10 major regression). The notation that was intended as a teaching device became a syntactic pattern the model imitated. User direction "go as recommended" → Option A revert: rolled back Rule 4 sentence-append + Example 12 + header bump + Recommended-fields block; canonical baseline stays at `368d565` Step 3.5 capture (7.4/10 · 80% pass · ship-confidence floor preserved). V-AI-EVAL-16/17 + V-CHAT-D-4 RETIRED (contract surface no longer exists). V-AI-EVAL-14 Guard 3 future-proofing (count >= 11) STAYS (correct discipline regardless of Examples 12+ existence). **Learning captured: behavior examples MUST NOT use inline `*[invokes X(...)]*` notation that LLMs can syntactically imitate; future arcs adding examples should use descriptive prose ("The chat then invokes the X tool with these args: ...") or out-of-band depiction.** R1+R2 residuals remain · addressable at Step 5 (preview-modal UX surfaces blanks visibly · engineer fills before apply) rather than in Layer 1 prompt-text tuning. Step 4 user-facing impl preamble is UNBLOCKED at the restored Step 3.5 baseline.

#### S20.4.1.4 · chat-says-vs-chat-does guard at chatService layer (additive 2026-05-14 late evening · Sub-arc D Step 3.9 · LOCKED post-Step-3.8 revert)

Defensive UX detection layer at `services/chatService.js` output time. Detects the hallucination pattern documented across 4 baselines (d73ce60 · cdd367a · 5466ea3 · ae33705): chat writes prose claiming an action was taken ("I've proposed X" · "Done. I've proposed Y" · "Proposal submitted ✓" · "now proposed for your review") while `proposedActions: []` (no tool fired). The pattern is structural LLM inconsistency under cognitive load · NOT a prompt-text-fixable issue (prompt-text iteration cycle Step 3.5 → 3.6 → 3.7 → 3.8 confirmed this · 2 of 4 iterations regressed · volatility exceeded the lift signal).

**Strategic pivot rationale**: shift defensive UX to the application layer rather than continue prompt-tuning. The guard is deterministic · 100% reproducible · costs nothing at runtime · works regardless of which LLM provider is configured.

**Guard mechanism** (Step 3.9 impl):

1. After `visibleResponse` is finalized (post-handshake-strip + post-UUID-scrub) and after the `groundingViolations` check but before the envelope is returned, evaluate the conjunction guard:
   - Condition: `proposedActions.length === 0` AND `HALLUCINATION_RE.test(visibleResponse)`
   - `HALLUCINATION_RE` covers: `(?:I'?ve|I\s+have)\s+(?:propose[ds]?|added|submitted|captured|marked|created)` and `Proposal\s+(?:submitted|is\s+in\s+your\s+panel|is\s+ready|recorded)` and `now\s+propose[ds]?\s+for\s+your\s+review`
2. On detection, construct `proposalEmissionWarning = { detected: true, matchedPhrase: <the first regex match>, reason: "Chat described an action in prose without invoking the proposeAction tool. Engineer review recommended." }`.
3. Otherwise: `proposalEmissionWarning = null`.
4. The envelope shape extends: `{ response, provenance, contractAck, groundingViolations, proposedActions, proposalEmissionWarning }`.

**False-positive avoidance**: the regex deliberately excludes:
- Bare "Captured." alone (legitimate when chat actually fires the tool · ACT-INST-CUR-2/3 use this preamble + actually fire)
- Tutorial-mode responses like "Got it · that's a custom instance · here's how to capture it manually" (ACT-INST-CUR-4 pattern · different failure class)
- Generic agreement phrases ("That's two clear signals" · "I'd be happy to help")

**Downstream consumers**:
- Eval harness reads `envelope.proposalEmissionWarning` and includes it in the per-case result (forensic signal for future baselines · helps distinguish "chat declined correctly" from "chat hallucinated firing")
- Step 5 preview modal (future) renders the warning as: *"⚠ The chat described an action but did not emit a structured proposal. Manual entry required?"*
- Anti-pattern: silently dropping the warning · the engineer-facing UI MUST surface it on detection

**Schema-conditional**: the regex covers the dominant hallucination patterns observed across 4 baselines (d73ce60 · cdd367a · 5466ea3 · ae33705). Future arcs may discover new phrasings (e.g. "Logged" · "Action queued") · the regex can be extended in-place as new evidence surfaces · NO constitutional re-preamble required for regex-pattern additions provided the envelope shape stays.

**Cross-references**:
- TESTS §T48 — V-CHAT-D-5 (3 guards · source-grep regex + envelope field + conjunction logic).
- RULES §16 CH38(b) — envelope shape contract amended to include `proposalEmissionWarning`.
- §S20.4.1.3 — the parent stub-emission contract that this guard layers on top of.
...
| 2026-05-14 | Session-handover infrastructure (rc.9-dev) | Doc-only restructure. NEW `docs/HANDOVER_TEMPLATE.md` v1.0 — meta-discipline audit synthesizing the 5 foundational discipline layers (Architectural integrity · Constitutional governance · Spec-driven development · Real-only validation · No-patches) from ~25 anchor docs into a canonical session-handover skeleton (`e058c57`). `HANDOFF.md` rewritten from the template (797 → 224 lines; historical commit ledger relocated to `docs/SESSION_LOG_*.md` files per separation-of-concerns) (`87fafc8`). NEW `docs/SESSION_PRIMING_PROMPT.md` v1.0 — one-shot pointer for fresh-context sessions: instructs the model to read HANDOFF.md + ack with a structured 5-discipline summary + 7 NEVER rules before any code work (`d9bca36`). NO production code changes; banner unchanged at 1292/1292 GREEN. Authority: `feedback_principal_architect_discipline.md` (TIER-1 anchor that the template synthesizes). |
| 2026-05-14 | v3.0.0-rc.8 git tag retroactively created on commit `a322262` | The rc.8 release-close commit `a322262` was pushed 2026-05-13 evening, but the `git tag v3.0.0-rc.8` command was never executed in lock-step. The APP_VERSION ledger had already claimed "TAGGED 2026-05-13"; the GitHub Pages deploy chip showed `Canvas v3.0.0-rc.8`; but `git tag -l` and `gh release list` returned no rc.8 tag. Corrective action 2026-05-14 morning: created annotated `v3.0.0-rc.8` tag on `a322262` with full release-notes message + pushed to origin (`03b00b7`). Coherence restored between ledger/Pages-chip claims and the git tag-graph reality. No SPEC contract change; recorded here as audit trail for the rc.8 → rc.9 transition. |
| 2026-05-14 | APP_VERSION 3.0.0-rc.8 → 3.0.0-rc.9-dev (R30.1) | First commit past the (retroactively-created) rc.8 tag MUST add the `-dev` suffix per `R30.1` + PREFLIGHT 1a (`f70cc38`). V-VERSION-2 source-grep regression guard verified. Ledger block at top of `core/version.js` comment header lists the rc.9 in-flight scope: Sub-arc C knowledge-base wiring · Sub-arc D action proposals (deferred) · BUG-061 Save-draft lifecycle (deferred) · BUG-063 engagement-init residual fields · BUG-052 modal-residue test flake · `gap.closeReason` doc-drift. |
| 2026-05-14 | Eval baseline JSON captured into the repo (rc.9-dev) | Closes the discipline gap surfaced 2026-05-14 morning during Sub-arc C scoping: HANDOFF + RELEASE_NOTES_rc.8.md + `core/version.js` ledger + `SUB_ARC_B5_DOC_AUDIT_GAP_LIST.md` all cited `tests/aiEvals/baseline-2026-05-13T20-01-50-669Z.json` as the rc.8 measurement bar, but the file itself was never committed (only `baseline.json.HOWTO.md` + the harness lived in the repo). Both `tests/aiEvals/baseline.json` (canonical name per the harness's evalRunner convention) and the timestamped historical record committed in `9e85543` (same 175KB content; checksum 67243fa49c82cd12). Aggregate verified: 9.16/10 avg · 96% pass · 24/25 with GRD-2 failing 5/10 + judge verdict *"enumerated counts and layer distribution are not grounded"* (the proximate evidence that motivated Sub-arc C Rule 10). |
| 2026-05-14 | §S20.4.1.2 NEW · Rule 10 (Quantitative honesty) + Examples 9 + 10 · RULES §16 CH37 NEW · ROADMAP.md NEW | **Sub-arc C Commit A** (`37356bd`) — `[CONSTITUTIONAL TOUCH PROPOSED]` Q&A flow per Rule A; user explicit approval ("Go with all proposed answers" · 10 design questions). §S20.4.1.2 NEW: Rule 10 (schema-truthful enumeration) is a behavior contract grounded in schema state — the v3 install-base schema collects names + types + descriptions but NOT quantities, so chat enumerates items by name (never percentage / weighted aggregate / capacity-share across instance rows). Row-counts permissible only with explicit "how many" + source-tool citation + row-count qualification. Schema-conditional: rule narrows automatically when a `quantity` field is added to a layer's instance schema (future feature per `docs/ROADMAP.md`). Examples 9 (per-entity drilldown via `selectLinkedComposition`) + 10 (enumerate-by-name pattern, cites Rule 10 inline so enforcement is traceable in chat output) ship alongside. RULES §16 CH37 NEW: schema-truthful enumeration contract + anti-pattern guards. ROADMAP.md NEW (first deferred-feature entry: quantity-collection at install-base layer, v3.1.0 candidate). 3 RED-first tests: V-AI-EVAL-6 (Rule 10 source-grep · 4 guards) + V-AI-EVAL-7 (Example 9 source-grep · 3 guards) + V-AI-EVAL-8 (Example 10 source-grep + inline Rule 10 citation · 3 guards). Banner 1292 → 1295 (3 RED captured). |
| 2026-05-14 | §S20.4.1.2 impl · Layer 1 Role section gains Rule 10 + Examples 9 + 10 | **Sub-arc C Commit B** (`2f3176f`) — `services/systemPromptAssembler.js` Layer 1 Role section gains Rule 10 (~200 tokens) + Example 9 (per-entity drilldown · ~80 words) + Example 10 (enumerate-by-name · cites Rule 10 inline · ~120 words). Example header bumped from "6 worked examples" → "10 worked examples". Note footer extended to mention `selectLinkedComposition` + Rule 10's percentage prohibition. Total ~450 tokens added to Layer 1 (cached prefix; within S20.6 + S20.7 budget). V-AI-EVAL-6/7/8 flip RED → GREEN. Banner 1295/1295 GREEN. Direct prompt-assembly check via dynamic import confirmed all 8 sanity assertions (rule present · quantitative-honesty phrase · Example 9 + 10 markers · tool-call demos · inline Rule 10 citation · 10-rule count). |
| 2026-05-14 | Sub-arc C user-facing reference docs (rc.9-dev) | **Sub-arc C Commit C** (`595264f`) — 3 of 4 originally-proposed docs ship; matrix doc deferred to rc.10 per SME blocker. NEW `docs/CANVAS_CHAT_USER_GUIDE.md` (~108 lines · when to use the chat · 6 question-type sections · tips · provider config). NEW `docs/CURRENT_VS_DESIRED_STATE.md` (~55 lines · Tab 2 vs Tab 3 conceptual anchor · disposition lifecycle table · originId mechanics · common confusions). NEW `docs/GAP_TYPE_VS_DISPOSITION.md` (~80 lines · gapType-is-the-work / disposition-is-the-outcome distinction · worked example · asymmetric cases). DEFERRED to rc.10: `docs/DELL_SOLUTIONS_BY_GAP_TYPE_AND_LAYER.md` (gap_type × layer × Dell product matrix; needs SME input). Doc-only; banner unchanged at 1295/1295 GREEN. |
| 2026-05-14 | rc.9 post-Sub-arc-C eval re-baseline (DIAGNOSTIC ONLY) | **Sub-arc C eval re-capture** (`0e3d0f6`) — user ran `window.runCanvasAiEvals({ judgeProviderKey: "anthropic" })` post-Sub-arc-C against same provider/model/judge as rc.8. Results captured as `tests/aiEvals/baseline-2026-05-14T11-48-56-300Z.json`; **NOT** overwriting `baseline.json` (rc.8 ref preserved pending forensics). Aggregate: 8.92/10 avg · 88% pass (22/25) · 3 fails. WIN on Sub-arc C target: GRD-2 5/10 → 8/10 (+3 · the case Rule 10 + Example 10 were designed for); data-grounding category 8.40 → 9.60 (+1.20). REGRESSIONS traced to BUG-063 manifestation + adjacent honesty gap: DSC-4 10 → 4 (chat saw `customer.vertical = "Financial Services"` default as real); APP-4 10 → 5 (chat saw `customer.name = "New customer"` placeholder as real); APP-5 10 → 5 (chat used loaded demo data as illustrative example without disclaimer). Diagnostic-only commit; rc.8 baseline remained canonical pending Phase 1 fixes. |
| 2026-05-14 | BUG-063 fix · schema/customer.js + schema/engagement.js empty-state defaults | **BUG-063 closure** (`b70a96d` preamble + RED + BUG_LOG status flip · `9f8436f` impl + schema relax + collateral test updates). Root cause: `CustomerSchema.name` + `CustomerSchema.vertical` carried `.min(1)` constraints, forcing `createEmptyCustomer()` to pick real-looking placeholder defaults ("New customer" / "Financial Services" / "EMEA"). The chat read these as real customer data, fabricating answers in empty-engagement test cases (rc.9 baseline DSC-4 + APP-4 regressions). Fix: relaxed `name` + `vertical` to `z.string()` (no min); flipped factory defaults to `""`. Empty string is the canonical "unset" sentinel post-fix. NEW V-FLOW-INIT-CLEAR-1/2 regression guards. 3 collateral test updates: V-SCH-11 retired-with-inversion (contract inverted from "rejects empty" to "accepts empty"); V-PATH-16 + V-SEL-6a fixtures flipped from implicit-default to explicit-override. Banner 1295 → 1297 GREEN (Docker nginx canonical env; +2 net tests). Severity escalated from Low-Medium to Medium-High at fix time given the eval-honesty cost. UI follow-up (ContextView.js dropdown placeholder rendering for empty vertical) deferred to UX polish pass; NOT rc.9 blocker. |
| 2026-05-14 | rc.9 eval re-baseline validates BUG-063 fix · NEW measurement bar | **Sub-arc C eval re-baseline GREEN** (`8aac4c5`) — user re-ran eval with BUG-063 fix landed. Results: **9.32/10 avg · 100% pass rate (25/25) · 0 fails**. `tests/aiEvals/baseline.json` overwritten to point at this snapshot (per HOWTO.md step 7 + README convention); timestamped historical record at `baseline-2026-05-14T12-19-31-211Z.json`. Three-way vs rc.8: avg 9.16 → 9.32 (+0.16); pass rate 96% → 100% (+4pp); data-grounding 8.40 → 9.40 (+1.00 · Sub-arc C target hit); honest dim 1.92 → 1.96 (+0.04 · BUG-063 fix lift); failed 1 → 0. Per-case prediction validation: DSC-4 4 → 9 ✅ (predicted 9-10); APP-4 5 → 10 ✅ (predicted 9-10); APP-5 5 → 8 ✅ (cleared without Rule 10a · which was a contingent next-step); GRD-2 8 → 7 (slight slip, still passes; rc.8 was 5). Rule 10a stays on rc.10 candidate list if APP-5's residual "inventing example" pattern persists in future re-captures. |
| 2026-05-14 | RELEASE v3.0.0-rc.9 | **TAGGED 2026-05-14.** Closes the rc.9 arc. Banner **1297/1297 GREEN ✅** (was 1292 at rc.8; +5 net tests: V-AI-EVAL-6/7/8 + V-FLOW-INIT-CLEAR-1/2). Eval baseline: **9.32/10 avg · 100% pass rate** on 25-case golden set (25/25). Rolled in: handover-template adoption (`HANDOVER_TEMPLATE.md` v1.0 + `HANDOFF.md` rewrite + `SESSION_PRIMING_PROMPT.md` v1.0) · retroactive rc.8 git tag creation · APP_VERSION rc.8 → rc.9-dev → rc.9 · eval baseline JSON captured into repo · **Sub-arc C** (Rule 10 quantitative honesty + Examples 9 + 10 + 3 author docs + ROADMAP) · **BUG-063 fix** (schema relax + factory defaults flip · cleared DSC-4 + APP-4 + APP-5 regressions). SPEC annexes added: §S20.4.1.2 (Rule 10 + Examples 9-10). RULES added: §16 CH37 (schema-truthful enumeration contract). New docs: `docs/CANVAS_CHAT_USER_GUIDE.md` · `docs/CURRENT_VS_DESIRED_STATE.md` · `docs/GAP_TYPE_VS_DISPOSITION.md` · `docs/ROADMAP.md`. New tests: V-AI-EVAL-6/7/8 (Rule 10 + Examples 9/10 source-grep) + V-FLOW-INIT-CLEAR-1/2 (BUG-063 regression guard). Deferred to rc.10: Sub-arc D action proposals (D-Rule LOCKED) · `DELL_SOLUTIONS_BY_GAP_TYPE_AND_LAYER.md` matrix (SME) · Rule 10a (APP-5 residual concern, contingent) · BUG-061 Save-draft lifecycle · BUG-052 modal-residue · `gap.closeReason` doc-drift · ContextView.js empty-state dropdown rendering. See `docs/RELEASE_NOTES_rc.9.md` for full per-arc commit ledger. |
| 2026-05-14 evening | §S20.4.1.3 Step 3.5 amendment · Rule 4 verb-strength tightening + Example 11 + proposeAction tool description hardening + ACT-INST-CUR-1 fixture fix | **Sub-arc D Step 3.5 preamble** (`<this commit>`) — `[CONSTITUTIONAL TOUCH PROPOSED]` Q&A flow per Rule A; user explicit approval ("Go with all recommendations" · 9 design questions Q1..Q9). Amendment row appended to §S20.4.1.3 documenting the Rule 4 verb-strength bump from "invoke" to "MUST invoke" + "contract violation" framing + Example 11 (canonical add-driver tool-use pattern · 10 → 11 worked examples header bump) + chatTools.js proposeAction description hardened with "MUST be invoked" + "closeReason ... REQUIRED on close-gap" callout. Motivated by rc.10 first action-correctness baseline (`d73ce60` · `tests/aiEvals/baseline-action.json` · 3.8/10 avg · 40% pass · 3 of 4 emit-cases scored 0/10 by no-emission · ACT-INST-DES-1 specifically hallucinated "Proposal submitted ✓" in prose). RULES §16 CH38(a) amendment row paired. ACT-INST-CUR-1 golden-set fidelity bug (demo:northstar-health ships Veeam pre-populated · contradicting the case hint) addressed by prompt rewrite to use Commvault HyperScale X at Branch Clinic (non-pre-populated vendor + env combo). 4 RED-first tests: V-AI-EVAL-13 (Rule 4 "MUST invoke" + "contract violation" source-grep · 2 guards) + V-AI-EVAL-14 (Example 11 + "11 worked examples" header + add-driver tool-use pattern · 3 guards) + V-CHAT-D-3 (proposeAction description "MUST be invoked" + closeReason REQUIRED callout · 2 guards) + V-AI-EVAL-15 (ACT-INST-CUR-1 no-Veeam + Commvault HyperScale X + Branch Clinic source-grep · 3 guards). Banner 1303 → 1307 (4 RED captured at first guards). Expected lift target post-impl: per-category 0/0/0/9/10 → ~6/6/6/9/10 · passRate 40% → ≥80% · avg 3.8 → ≥7.0. Step 4 user-facing impl (preview modal + Apply button + CH26 + §S25 aiTag.kind ext + Mode 1 surface) BLOCKED on Step 3.5 landing + re-baseline confirming lift. |
| 2026-05-14 evening | Step 3.5 re-baseline · rc.10 action-correctness measurement bar UPDATED · target HIT | **Step 3.5 close** (`368d565`) — user-run re-baseline against d73ce60 with the Rule 4 + Example 11 + tool description + fixture changes applied. Aggregate: 3.8 → 7.4 avg (+3.6) · 40% → 80% pass (+40pp · 4/5 cases) · per-dimension lifts: actionKind 0.8→1.6 · targetState 0.8→1.4 · payloadAccuracy 0.6→1.2 · confidenceCalibration 0.8→1.6 · restraint 0.8→1.6. Per-category: add-driver 0→9 (clean fix) · add-instance-current 0→0 (RESIDUAL · new ask-permission failure mode) · add-instance-desired 0→8 (clean fix) · close-gap 9→10 (Q3 closeReason callout worked) · restraint 10→10 stable. Step 3.5 aggregate target HIT (avg ≥7.0 + passRate ≥80%); Step 4 user-facing impl UNBLOCKED. Two residuals (R1 ACT-INST-CUR-1 ask-permission + R2 payload optional-field completeness) flagged for Step 3.6 micro-arc before Step 4 lands. `tests/aiEvals/baseline-action.json` canonical updated to point at this snapshot; d73ce60 baseline preserved as `baseline-action-2026-05-14T15-51-12-201Z.json` timestamped historical (pre-Step-3.5); new `baseline-action-2026-05-14T16-26-51-443Z.json` is the post-Step-3.5 historical record. |
| 2026-05-14 evening | §S20.4.1.3 Step 3.6 amendment · Rule 4 no-ask-permission sentence-append + Example 12 + proposeAction "Recommended fields per kind" block | **Sub-arc D Step 3.6 preamble** (`<this commit>`) — `[CONSTITUTIONAL TOUCH PROPOSED]` Q&A flow per Rule A; user explicit approval ("Go with all recommendations" · 6 design questions Q1..Q6) · scope pre-authorized by user direction "recommendation: B" after the 368d565 forensic analysis. Step 3.5 hit aggregate target (avg 7.4 · 80% pass) but surfaced 2 residuals: R1 ask-permission pattern on ACT-INST-CUR-1 (chat asks "Would you like me to propose...?" instead of just emitting · 0/10) + R2 payload optional-field completeness (ACT-DRIVER-1 9/10 missing priority · ACT-INST-DES-1 8/10 missing vendor/vendorGroup/disposition/originId). Step 3.6 closes both with: Rule 4 sentence-append forbidding ask-permission ("Asking the engineer permission to invoke proposeAction ... is the same contract violation as describing the action in prose; ... Just invoke the tool.") + proposeAction tool description "Recommended fields per kind" block (priority on add-driver; vendor + vendorGroup + criticality on add-instance-current; vendor + vendorGroup + disposition + originId on add-instance-desired) + Example 12 NEW (canonical add-instance-current pattern · Veritas NetBackup at DR Site · intentionally distinct from ACT-INST-CUR-1's Commvault/Branch-Clinic fixture to prevent memorized-text matching). RULES §16 CH38(a) sub-rule extended in parallel. 3 RED-first source-grep tests: V-AI-EVAL-16 (Rule 4 no-ask-permission · 2 guards) + V-AI-EVAL-17 (Example 12 marker + add-instance-current tool-use + "12 worked examples" header · 3 guards) + V-CHAT-D-4 ("Recommended fields per kind" + priority + vendorGroup + disposition · 2 guards). Banner 1307 → 1310 (3 RED captured at first guards). Expected lift target post-impl: per-category 9/0/8/10/10 → ~9/8/9/10/10 · passRate 80% → ≥90% · avg 7.4 → ≥8.5. Step 4 user-facing impl preamble UNBLOCKED post-Step-3.5 · Step 3.6 is a polish micro-arc bringing ship-confidence higher before Step 4 lands. |
| 2026-05-14 evening | Step 3.6 REVERTED · re-baseline regression caused by Example-notation imitation | **Step 3.6 revert** (`<this commit>`) — Step 3.6 re-baseline (`tests/aiEvals/baseline-action-2026-05-14T17-44-07-909Z.json` · DIAGNOSTIC only · `cdd367a` commit) showed avg 7.4 → 6.0 · passRate 80% → 60% (avg regression −1.4 · pass −20pp). Forensic root cause: Example 12's inline `*[invokes proposeAction(kind='add-instance-current'...)]*` notation was IMITATED as prose by the LLM (ACT-INST-CUR-1 chat answer wrote the bracketed text literally in response body · `proposedActions: []`) AND ACT-INST-DES-1 hallucinated "I've proposed..." with empty proposedActions (8/10 → 0/10 major regression). The notation that was intended as a teaching device became a syntactic pattern the model imitated. User direction "go as recommended" → Option A revert: rolled back systemPromptAssembler.js Rule 4 sentence-append + Example 12 + "12 worked examples" header bump → restored to Step 3.5 state (Rule 4 with MUST invoke + Example 11 + "11 worked examples"); rolled back chatTools.js "Recommended fields per kind" block → restored Step 3.5 description. V-AI-EVAL-16/17 + V-CHAT-D-4 RETIRED (3 tests removed from appSpec.js · their contract surfaces no longer exist · 3 V-* IDs are now historical record only). V-AI-EVAL-14 Guard 3 future-proofing (count >= 11 instead of count = 11) STAYS (correct discipline regardless · the Step 3.5 commit body 5ec0a65 predicted this case). Canonical `tests/aiEvals/baseline-action.json` NOT touched · still points at `368d565` Step 3.5 capture (7.4/10 · 80% pass · ship-confidence floor preserved). Banner 1310/1310 → 1307/1307 GREEN (−3 retired tests). Learning captured in CH38(a) extension: behavior examples MUST NOT use inline `*[invokes X(...)]*` notation. R1+R2 residuals from Step 3.5 remain · addressable at Step 5 (preview-modal UX) rather than in Layer 1 prompt-text tuning. Step 4 user-facing impl preamble UNBLOCKED at the restored Step 3.5 baseline. |

...
| V-FLOW-GROUND-FAIL-1 | **REWRITTEN Sub-arc B 2026-05-13** · `services/groundingVerifier.js` `verifyGrounding(response, engagement)` flags a response that references a gap description not in the engagement; populates `violations[*].kind === "gap-description"` AND `violations[*].severity === "high"`; `ok === false`. (Verifier still returns ok:false; chatService no longer replaces the response — annotation envelope only, per R37.6 SOFT-WARN rewrite.) |
| V-FLOW-GROUND-FAIL-2 | **REWRITTEN Sub-arc B 2026-05-13** · Verifier flags a response that references a vendor name not in the engagement AND not in `DELL_PRODUCT_TAXONOMY`; populates `violations[*].kind === "vendor"` AND `violations[*].severity === "medium"`. (Vendor severity is medium because the name could be a Dell-catalog reference; clearer cuts get high.) |
| V-FLOW-GROUND-FAIL-3 | **REWRITTEN Sub-arc B 2026-05-13** · Verifier flags the Local-B regression case (project plan with "PowerScale F710 & XE9680 (Q2 close)" / "Executive Review (June 30) Phase 1 progress" — phases/dates not in engagement). Populates `violations[*].kind` ∈ `{"date-deliverable", "project-phase"}` AND `violations[*].severity === "low"` (v3 schema does not yet carry phase/date fields, so the reference is informational rather than verified hallucination). |
| V-FLOW-GROUND-FAIL-4 | **EXTENDED Sub-arc B 2026-05-13** · Source-grep assertion on `services/chatService.js streamChat(...)`: MUST contain a call to `verifyGrounding(...)` AND MUST NOT contain the retired render-error template phrase ("The model produced an answer with claims that don't trace to the engagement" — that exact string indicates the BLOCK behavior is still in place). The retired template is RETIRED per R37.6 SOFT-WARN rewrite. |
| V-FLOW-GROUND-FAIL-5 | Catalog-whitelist invariant: a response that mentions a Dell product (DELL_PRODUCT_TAXONOMY entry) NOT in the engagement does NOT trigger a violation (catalog reference data is allowed even if not in engagement). Same for BUSINESS_DRIVERS, ENV_CATALOG labels — these are reference data, not hallucination. (UNCHANGED Sub-arc B — this was already a pass case.) |
| V-FLOW-GROUND-ANNOTATE-1 | **NEW Sub-arc B 2026-05-13** · Source-grep assertion on `services/chatService.js`: the `onComplete` envelope MUST carry a `groundingViolations` field, AND the `result.response` / visible-response variable MUST NOT be reassigned to the retired render-error template (the SOFT-WARN integration preserves the response unchanged and passes violations alongside it). |
| V-FLOW-GROUND-ANNOTATE-2 | **NEW Sub-arc B 2026-05-13** · DOM contract test on `ui/views/CanvasChatOverlay.js`: when `streamChat` `onComplete` fires with a non-empty `groundingViolations` array, the assistant bubble renders a footer block (e.g. `.canvas-chat-grounding-annotations` or equivalent `[data-grounding-annotations]` element) containing one annotation row per violation, with severity-tier classes (`.severity-high` / `.severity-medium` / `.severity-low`). |

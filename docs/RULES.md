...
| CH38 (amended Step 3.5 + Step 3.7 + Step 3.9 2026-05-14 evening · PIVOTED 2026-05-15 post-calibration · Steps 3.6 + 3.8 attempted then REVERTED · D4 25-case expansion captured · prompt-text iteration cycle closed at diminishing returns · Sub-arc D ARCHITECTURE PIVOTED to engineer-issued import via Path B per framing-doc A19 · proposeAction tool stays but its purpose narrows to chat-quality measurement + Mode 2 optional fallback · primary UX path is now Workshop Notes overlay → Path B importer per SPEC §S20.4.1.5) | Structured action-proposal contract (Sub-arc D stub-emission, per SPEC §S20.4.1.3, rc.10 LOCKED 2026-05-14 evening · sub-rule (a) verb-strength tightened Step 3.5 · Step 3.6 verbose-notation Example 12 REVERTED · Step 3.7 Example 12 retry + Example 11 rewrite with Example-8-style short-form notation · Step 3.8 Example 13 add-instance-desired addition REVERTED post-20:19 regression · prompt-text iteration cycle complete · strategic pivot to app-layer chat-says-vs-chat-does guard at Step 3.9 + eval-methodology calibration) — Layer 1 Rule 4 is AMENDED to operationalize the "propose then apply" pattern via tool-use: when a proposal matches a known action-kind, the chat MUST invoke the `proposeAction` tool (registered in `services/chatTools.js`) with structured args validated by `schema/actionProposal.js`; the proposal is captured in `chatService.streamChat`'s envelope `proposedActions[]` field. (a) **Tool-use mechanism** — `proposeAction` is a chat tool with input_schema derived from `schema/actionProposal.js ActionProposalSchema` (single source of truth · JSON Schema generated from Zod; no manual duplication). The chat may invoke `proposeAction` multiple times in one turn for multi-proposal scenarios; each call appends to `proposedActions[]`. **VERB-STRENGTH AMENDMENT (Step 3.5 2026-05-14 evening · post-d73ce60 baseline)**: Layer 1 Rule 4 was originally phrased as "invoke the `proposeAction` tool" (advisory verb). The rc.10 first action-correctness baseline (`tests/aiEvals/baseline-action.json` · 3.8/10 avg · 40% pass rate) revealed 3 of 4 emit-cases scored 0/10 by no-emission (chat described actions in prose without invoking the tool), with ACT-INST-DES-1 specifically hallucinating "Proposal submitted ✓" in prose while `proposedActions: []`. Rule 4 is amended to imperative verb-strength: "you **MUST** invoke the `proposeAction` tool ... Describing the action in prose without invoking the tool is a **contract violation**; the engineer-facing UI only sees proposals emitted through the tool call." The tool description in `services/chatTools.js` is similarly amended with "MUST be invoked" + per-kind required-field callout (specifically: `closeReason` field marked REQUIRED on `close-gap` kind · addressing the ACT-CLOSE-1 9/10 case where evidence-of-closure was put in `rationale` instead of `payload.closeReason`). Behavior Examples section in §S20.4.1 gains Example 11 (canonical add-driver tool-use pattern · header count bumped 10 → 11 worked examples). NO plumbing changes (tool name unchanged · envelope unchanged · `ActionProposalSchema` unchanged · 4-kind enum unchanged · CH38 sub-rules (b)..(g) unchanged); only steering text tightens. **STEP 3.6 EXTENSION ATTEMPTED then REVERTED (2026-05-14 evening · post-368d565 + post-cdd367a baselines)**: Step 3.5 hit aggregate target (avg 7.4 · 80% pass) but surfaced two residuals: (R1) ask-permission pattern on ACT-INST-CUR-1 — chat asked "Would you like me to propose...?" instead of just emitting on unambiguous v1-kind input · (R2) payload optional-field completeness gap (ACT-DRIVER-1 9/10 missing `priority`; ACT-INST-DES-1 8/10 missing `vendor` + `vendorGroup` + `disposition` + `originId`). Step 3.6 ATTEMPTED to close both: Rule 4 sentence-append forbidding ask-permission + `proposeAction` "**Recommended fields per kind**" block + Example 12 (canonical add-instance-current tool-use pattern · Veritas NetBackup at DR Site · 11 → 12 worked examples header bump). **REVERTED 2026-05-14 evening per re-baseline (`baseline-action-2026-05-14T17-44-07-909Z.json` DIAGNOSTIC only · `cdd367a`)**: regression captured — avg 7.4 → 6.0 · passRate 80% → 60% · per-cat add-instance-desired 8 → 0 (catastrophic). Root cause: Example 12's inline `*[invokes proposeAction(...)]*` notation was IMITATED by the LLM as prose text (ACT-INST-CUR-1 chat answer literally wrote the bracketed notation in its response body; ACT-INST-DES-1 hallucinated "I've proposed..." with empty proposedActions). User direction "go as recommended" → Option A revert: systemPromptAssembler.js Rule 4 sentence-append + Example 12 + "12 worked examples" header bump rolled back to Step 3.5 state; chatTools.js "Recommended fields per kind" block rolled back. V-AI-EVAL-16/17 + V-CHAT-D-4 RETIRED (contract surface no longer exists). V-AI-EVAL-14 Guard 3 future-proofing (count >= 11) STAYS (correct discipline regardless). Canonical `tests/aiEvals/baseline-action.json` UNCHANGED · still points at `368d565` Step 3.5 capture (7.4/10 · 80% pass · ship-confidence floor preserved). **Learning captured: behavior examples MUST NOT use inline `*[invokes X(...)]*` notation that LLMs can syntactically imitate; future arcs adding examples should use descriptive prose ("The chat then invokes the X tool with these args: ...") or out-of-band depiction.** R1+R2 residuals remain · addressable at Step 5 (preview-modal UX surfaces blanks visibly · engineer fills before apply) rather than in Layer 1 prompt-text tuning. Anti-pattern: inline LLM-imitable notation in behavior examples (the Step 3.6 lesson · permanently locked). **STEP 3.7 EXTENSION (2026-05-14 evening · post-5466ea3 25-case baseline)**: D4 expansion (5 → 25 cases) revealed add-instance-current as SYSTEMICALLY broken (1/5 pass · 4 emit-cases scored 0/10 by no-emission · including the gold-standard ACT-INST-CUR-3 VMware vSphere 7 input). Root cause: Layer 1 has Example 11 for add-driver but no equivalent worked example for add-instance-current. Step 3.7 retries Example 12 with PROVEN-SAFE short-form notation matching Example 8 (`*[calls X for Y: bullets]*` · NO inline args · NO function-call mirror syntax). Fixture: Veritas NetBackup at DR Site, tier-2, Medium criticality (distinct from ACT-INST-CUR-1's Commvault/Branch-Clinic fixture). Header count bumped 11 → 12 worked examples. V-AI-EVAL-19 negative guard forbids the Step-3.6 notation form (`*[invokes proposeAction(`) anywhere in source · forward-protection against future re-introduction. Expected lift target on 25-case set: add-instance-current 2.0 → ≥6.0 · passRate 68% → ≥80% · avg 6.68 → ≥7.5. **STEP 3.8 EXTENSION (2026-05-14 evening · post-3f8ff07 25-case re-baseline)**: Step 3.7 lifted aggregate (avg 6.68→7.4 · pass 68%→76%) and hit the add-instance-current target (2.0→6.0) but introduced a NEW regression on add-instance-desired (6.6→4.0 · ACT-INST-DES-1 7→0 + ACT-INST-DES-3 8→2 · both hallucinated "Done, I've proposed..." without firing the tool). The "every emit-kind needs an example" hypothesis is now well-grounded across 4 baselines: add-driver (Example 11) fires · add-instance-current (Example 12) fires · add-instance-desired (NO example) hallucinates · close-gap (NO example) fires but is structurally simpler. Step 3.8 adds Example 13 (canonical add-instance-desired replace-with-originId pattern · PowerScale H700 replacing HPE 3PAR at DR Site · safe short-form notation matching Examples 11+12 · intentionally distinct from ACT-INST-DES-1 fixture). Header count bumped 12 → 13 worked examples. Expected lift on 25-case canonical: add-instance-desired 4.0 → ≥7.0 · passRate 76% → ≥84% · avg 7.4 → ≥7.8. **STEP 3.8 REVERTED (2026-05-14 evening · post-20:19 25-case re-baseline regression)**: Step 3.8 impl + Example 13 addition produced a REGRESSION (avg 7.4 → 5.92 · pass 76% → 56% · add-instance-current 6.0 → 2.2 catastrophic). The "more examples are better" hypothesis FAILED at this Layer 1 density · cognitive crowding crashed the Example 12 win for add-instance-current. User strategic pivot: revert Step 3.8 + scaffold chat-says-vs-chat-does guard at chatService layer (defensive UX detects "I've proposed" prose with empty proposedActions[] · surfaces warning for engineer review) + calibrate eval methodology (Experiment A = sampling-noise · Experiment B = cross-judge Gemini test). Canonical baseline-action.json stays at 3f8ff07 (7.4/76%) as the last validated improvement. Prompt-text iteration cycle CLOSED · 4 iterations · 2 regressions · diminishing returns confirmed. Step 3.9 (defensive UX guard) is the new direction. (b) **Envelope shape** — `chatService.streamChat` returns `{response, provenance, contractAck, groundingViolations, proposedActions, proposalEmissionWarning}` where `proposedActions` is ALWAYS present (empty array when no `proposeAction` was called) AND `proposalEmissionWarning` is ALWAYS present (object on hallucination detection · null otherwise). Downstream consumers (eval harness, future preview modal) check `proposedActions.length > 0` to determine emission AND check `proposalEmissionWarning !== null` to surface the chat-says-vs-chat-does mismatch warning. **STEP 3.9 EXTENSION (2026-05-14 late evening · post-Step-3.8 revert)**: the envelope gains `proposalEmissionWarning` field via the chatService output-time defensive guard. The guard fires when `proposedActions.length === 0` AND `HALLUCINATION_RE.test(visibleResponse)` (regex covers the dominant hallucination phrases observed across 4 regression baselines: "I've proposed" · "I've added" · "Proposal submitted" · "Proposal is in your panel" · "now proposed for your review"). On detection, the envelope carries `proposalEmissionWarning = { detected: true, matchedPhrase: <the matched substring>, reason: "Chat described an action in prose without invoking the proposeAction tool. Engineer review recommended." }`. Otherwise `null`. The guard is DETERMINISTIC (regex-based · no LLM dependency) and works against all 3 provider kinds (anthropic / openai-compatible / gemini) identically. False-positive avoidance: the regex deliberately excludes bare "Captured." (legitimate when tool fires) · tutorial-mode responses · generic agreement phrases. Step 5 preview-modal renders the warning as "⚠ Chat described an action but did not emit a structured proposal. Manual entry required?". Anti-pattern: silently dropping `proposalEmissionWarning` in any downstream consumer · the engineer-facing UI MUST surface it on detection · eval harness MUST include it in per-case forensic results. Schema-conditional: future arcs may extend `HALLUCINATION_RE` as new phrasings are discovered · regex-pattern additions do NOT require a fresh constitutional re-preamble provided the envelope shape stays. **SUB-ARC D ARCHITECTURE PIVOT 2026-05-15 (user-approved "go Y" post-calibration commit `9edcb36`)**: the primary Sub-arc D UX path shifts from "chat autonomously emits proposeAction" → "chat structures workshop notes + engineer explicitly commands import via Path B importer (§S47)". The `proposeAction` tool + `proposedActions[]` envelope field + `proposalEmissionWarning` guard ALL STAY · their purpose narrows from primary-UX-path to: (1) chat-quality measurement via the 25-case action-correctness rubric · (2) Mode 2 optional fallback for engineers who prefer chat-inline emission · (3) defensive UX detection of chat-says-vs-chat-does mismatches in Mode 2. The PRIMARY UX path (Mode 1 · Workshop Notes overlay) feeds Path B importer via a NEW adapter (`services/workshopNotesImportAdapter.js` · non-constitutional · transforms overlay output into Path B's `ImportPreviewModal`-compatible input). Path B accepts TWO input sources: (a) file upload (original v1) · (b) Workshop Notes overlay output (new · Sub-arc D primary). Both feed the same ImportPreviewModal review + apply pipeline. The pivot is documented at SPEC §S20.4.1.5 NEW (this commit) + §S47 amendment (this commit) + framing-doc A19 (this commit) + the calibration evidence at tests/aiEvals/calibration-*-2026-05-14T21-*.json files (commit `9edcb36`). The pivot SUPERSEDES framing-doc A14 Q4 (Mode 2 first by default · now Mode 1 primary · Mode 2 optional/deferred). The rest of A14-A18 stays UNCHANGED as historical discipline trail. (c) **Stub scope** — at this rc.10 stub commit there is NO Apply button, NO preview modal, NO commit-function invocation, NO `aiTag` writes. Proposals are RECORDED only. The Apply-button + preview-modal + aiTag.kind extension (`"discovery-note"` / `"ai-proposal"`) land at Sub-arc D Step 4 (user-facing impl). (d) **v1 action kinds** — exactly 4 are valid: `add-driver` · `add-instance-current` · `add-instance-desired` · `close-gap`. `flip-disposition` is DEFERRED to v1.5 per cascade-risk (originId chains may break · linked gaps may need re-evaluation). Destructive kinds (`delete-*` · `hide-*` · `archive-*`) are DEFERRED to v1.5+ alongside cascade-confirmation UX. (e) **Source enum** — each ActionProposal carries `source: "discovery-note" | "ai-proposal"`. `"discovery-note"` is reserved for Mode 1 batch-process (note-taking overlay · ships at Step 4). `"ai-proposal"` is for Mode 2 conversational chat-inline (THIS stub). (f) **No-mocks** — every `proposeAction` invocation is a real-LLM tool call via the active provider (anthropic / openai-compatible / gemini); no scripted or stubbed substrate per `feedback_no_mocks.md`. (g) **Eval-harness integration** — `tests/aiEvals/evalRunner.js` extends to dispatch on `{ harness: "action-correctness" }` per SPEC §S48.3; runner captures `envelope.proposedActions` and passes to the §S48.2 judge. Anti-pattern: free-text JSON emission (markdown code fence) in lieu of `proposeAction` tool-use — chatService does NOT parse free-text proposals; multi-modal proposal (text + JSON in same response) — the chat emits chat text AND optional tool calls through the existing tool-use pipeline · NOT both; apply-button without engineer review at any layer in v1 (D-Rule LOCKED 2026-05-13 in `docs/SESSION_LOG_2026-05-13-final.md`); `aiTag.kind` enum extension before Step 4 (it requires a §S25 amendment which is bigger than the stub scope warrants); calling `proposeAction` with an action kind outside the 4 v1 kinds (the input_schema enum forbids it · the tool call rejects). | 🔴 HARD | tested by V-AI-EVAL-12 (evalRunner harness dispatch source-grep) + V-CHAT-D-1 (`proposeAction` tool registered in `chatTools.js` source-grep) + V-CHAT-D-2 (chatService envelope includes `proposedActions` field source-grep) + **Step 3.5 amendment vectors**: V-AI-EVAL-13 (Rule 4 "MUST invoke" + "contract violation" source-grep · 2 guards) + V-AI-EVAL-14 (Example 11 + "11 worked examples" header + add-driver tool-use pattern · 3 guards) + V-CHAT-D-3 (proposeAction tool description "MUST be invoked" + closeReason REQUIRED callout · 2 guards) + V-AI-EVAL-15 (ACT-INST-CUR-1 fixture fix: no-Veeam + Commvault HyperScale X + Branch Clinic · 3 guards) + **Step 3.6 amendment vectors RETIRED post-cdd367a regression revert**: V-AI-EVAL-16 + V-AI-EVAL-17 + V-CHAT-D-4 removed from `diagnostics/appSpec.js` because their contract surface (Rule 4 sentence-append + Example 12 + Recommended-fields block) was reverted. + **D4 25-case expansion + re-baseline** (`1168ab9` + `5466ea3`): canonical baseline transitioned to 25-case set per rc.8 sub-arc A.1→A.2 precedent · current canonical 6.68/10 · 68% pass on 25 cases (5466ea3) · add-instance-current category 1/5 pass revealed systemic failure. + **Step 3.7 amendment vectors**: V-AI-EVAL-18 (Example 12 retry with safe notation · 3 guards) + V-AI-EVAL-19 (negative guard against Step-3.6 imitation-hazard notation re-introduction · 1 guard) added at this preamble. Step 3.7 re-baseline 3f8ff07: avg 6.68→7.4 · pass 68%→76% · add-instance-current 2.0→6.0 (target hit) · add-instance-desired 6.6→4.0 (regression revealed). + **Step 3.8 amendment vector RETIRED post-20:19 re-baseline regression revert**: V-AI-EVAL-20 removed from `diagnostics/appSpec.js` because its contract surface (Example 13 add-instance-desired pattern) was reverted. V-AI-EVAL-18 Guard 3 floor (count >= 12) and V-AI-EVAL-14 Guard 3 floor (count >= 11) STAY (correct discipline regardless · independent of Step 3.8 revert). + **Step 3.9 amendment vector**: V-CHAT-D-5 (chat-says-vs-chat-does guard · 3 sub-guards · regex constant + envelope field + conjunction logic) added at the Step 3.9 preamble. Canonical baseline-action.json preserved at 3f8ff07 (7.4/10 · 76% pass · 25-case) as the ship-confidence floor · Step 3.9 is a defensive-UX scaffold that adds detection at chatService output-time WITHOUT altering the prompt-text layer (decoupling defensive UX from prompt-text iteration · which proved volatile at 4 iterations / 2 regressions) |

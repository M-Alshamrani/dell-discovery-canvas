...
| CH29 | Canvas AI Assistant header provider pills + footer breadcrumb + Cmd+K binding (per SPEC §S33) — the chat overlay's `head-extras` slot MUST render a `.canvas-chat-provider-pills` row enumerating every provider in `core/aiConfig.PROVIDERS` (one pill each; mock provider excluded). Each pill carries the provider label + a 6px status dot (green = ready / amber = needs key) + click semantics: inactive ready → switch active provider via `saveAiConfig`; inactive needs-key OR active → open Settings modal pre-focused on that provider's key field. Active pill: `var(--dell-blue)` filled + white text. The footer's `.canvas-chat-foot-lede` MUST update on every assistant `onComplete` to display the latest-turn provenance breadcrumb in the form `<provider-label> · <model> · <N> tokens · <ms>ms` (JetBrains Mono uppercase per §S32 R32.8); empty state reads "Ready". The Cmd+K / Ctrl+K shortcut (`wireAiAssistShortcut` in `app.js`) MUST call `openCanvasChat()` (Canvas AI Assistant), NOT `openAiOverlay()` (legacy v2.x AiAssistOverlay tile-grid). The legacy AiAssistOverlay is FLAGGED for retirement in Arc 4 (or sooner). Architectural fix for BUG-025. | 🔴 HARD | tested by V-PILLS-1..4 + V-FOOTER-CRUMB-1 + V-CMD-K-CANVAS-1 (rc.4-dev Arc 2) |

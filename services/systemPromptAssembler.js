...
function buildRoleSection(contractChecksum) {
  return [
    ...
    "Below are 13 worked examples of how to respond to the common families of questions the engineer asks. The examples are illustrative — do NOT mention them, label them, or narrate switching between 'modes'. Just respond in the same shape when the user's question matches a pattern.",
    "",
    ...
    "",
    "── Example 13 (workshop coaching · structured add-instance-desired emission via proposeAction tool · replace-with-originId pattern — added 2026-05-14 Sub-arc D Step 3.8 · use this pattern when the customer names a target Dell platform to REPLACE an existing instance. For greenfield introduction (no current platform to replace), omit originId and use disposition=introduce instead of disposition=replace. The proposeAction tool call IS the action; describing the proposal in prose without invoking the tool is a contract violation per Rule 4) ──",
    "User: \"Customer just decided they want to introduce PowerScale H700 for primary storage at the DR Site, replacing their existing HPE 3PAR array.\"",
    "Assistant: \"Captured as a desired-state replacement. *[calls proposeAction for PowerScale H700 replacing HPE 3PAR at DR Site: kind=add-instance-desired · layer=storage · vendor=Dell · vendorGroup=dell · disposition=replace · originId=<HPE-3PAR-current-uuid> · HIGH confidence]* The proposal is in your panel — once applied, the canvas will reflect PowerScale H700 as the desired-state replacement for HPE 3PAR in the DR Site. Next step: do you want me to also propose a gap to capture the migration work (data migration plan, performance validation, decommission of 3PAR), or is that already in scope of another gap?\"",

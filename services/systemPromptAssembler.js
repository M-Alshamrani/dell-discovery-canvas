...
function buildRoleSection(contractChecksum) {
  return [
    ...
    "",
    "── Example 12 (workshop coaching · structured add-instance-current emission · added 2026-05-14 Sub-arc D Step 3.6 · use this pattern when the customer names existing infrastructure at a specific environment. Do NOT ask permission to propose; invoke the tool immediately when input names a v1 kind · per Rule 4 the engineer's intent is established by the workshop flow itself) ──",
    ...
    "Assistant: \"Captured. *[invokes proposeAction(kind='add-instance-current', targetState='current', payload={layerId:'dataProtection', environmentId:'<DR-Site-uuid>', label:'Veritas NetBackup on HPE ProLiant DL360', vendor:'Veritas', vendorGroup:'nonDell', criticality:'Medium'}, confidence='HIGH', rationale='Customer explicitly named Veritas NetBackup at DR Site for tier-2 backup; tier-2 + DR-only scope indicates Medium criticality (not High like tier-1 clinical systems at the main DC).', source='ai-proposal')]* The proposal is in your panel — review the Medium criticality call (bump to High if their RPO/RTO target on this instance is tighter than I assumed). Next step: is the customer planning to consolidate this with their main DC backup, or keep it as a DR-tier-only system?\"",

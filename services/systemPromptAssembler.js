...
function buildRoleSection(contractChecksum) {
  return [
    ...
    "4. You may propose changes (rename, re-classify, re-link, add-instance, close-gap, etc.) but you may NOT mutate the canvas. When the proposal matches a known action-kind (per the action-proposal schema in SPEC §S20.4.1.3 + RULES §16 CH38), invoke the `proposeAction` tool with the structured args; the proposal is recorded for engineer review. For free-text proposals without a structured-action shape (rename ideas, qualitative re-classifications, architectural suggestions), end the chat response with 'click apply if you want me to open that view for you' as a navigation prompt.",
    ...
    "Below are 10 worked examples of how to respond to the common families of questions the engineer asks. The examples are illustrative — do NOT mention them, label them, or narrate switching between 'modes'. Just respond in the same shape when the user's question matches a pattern.",
    "",

...
export var ACTIONS = [
  ...
  },
  {
    id: "ops",
    label: "Operational",
    hint: "Process / operational change only.",
    icon: "~",
    gapType: "ops",
...
export function validateActionLinks(gap) {
  ...
  // case we can't pick a single rule to enforce, so skip.
  ...
  if (!c.ok) throw new Error("Action '" + action.label + "' " + c.message);
  var d = evaluateLinkRule(action.linksDesired, desiredCount, "desired");
  if (!d.ok) throw new Error("Action '" + action.label + "' " + d.message);
}
